...
public abstract class NettyRemotingAbstract {
	 ...
	 *
	 * @param permitsOneway
	 *            Number of permits for one-way requests.
	 * @param permitsAsync
	 *            Number of permits for asynchronous requests.
	 */
	 ...
	 * @param msg
	 *            incoming remoting command.
	 * @throws Exception
	 *             if there were any error while processing the incoming command.
	 */
	...
	public RemotingCommand invokeSyncImpl(final Channel channel, final RemotingCommand request,
		...
		try {
			...
			// 绑定opaque 和 responseFuture
			// 在processResponseCommand方法中收到请求响应后会设置RemotingCommand
			...
			// 如果在指定时间范围类返回的responseCommand为null
			// 可能原因:
			// 1.发送成功，但响应超时
			// 2.发送失败
	...
	public void invokeAsyncImpl(final Channel channel, final RemotingCommand request, final long timeoutMillis,
		...
		if (acquired) {
		    //包装成SemaphoreReleaseOnlyOnce，只能释放一次
